# PodHQ Client — Build History (Archive 2)

Reference-only, not `@`-included by CLAUDE.md — read on demand. Continues
from `ROADMAP-ARCHIVE.md` (Stages 1-9, the pilot era). This file covers
2026-08-16 (OWASP audit) through 2026-08-19 (the Wellness/Recovery Room
slot-duration fix), split out of `ROADMAP.md` on 2026-08-21 once that
file hit 60KB — well past Claude Code's ~15,000-character `@`-import
limit. `ROADMAP.md` picks up from the Stripe Connect Hove pilot
(2026-08-19) onward and is the active, auto-loaded log going forward.

If this file (or the next active `ROADMAP.md`) grows too large again,
split it the same way into a numbered `ROADMAP-ARCHIVE-3.md`, leave a
