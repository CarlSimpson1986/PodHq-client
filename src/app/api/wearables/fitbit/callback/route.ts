...
export async function GET(request: NextRequest) {
  ...
  const failureRedirect = () => {
    const response = NextResponse.redirect(new URL("/coach/profile?wearable=error", request.url));
  ...
  const response = NextResponse.redirect(new URL("/coach/profile?wearable=connected", request.url));
  response.cookies.delete(STATE_COOKIE);
  return response;
}
