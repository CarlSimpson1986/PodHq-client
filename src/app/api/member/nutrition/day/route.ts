...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = await checkRateLimit(user.id, "/api/member/nutrition/day");
  ...
  try {
    ...
    return NextResponse.json({ status: "ok", date, entries, totals, targets });
  } catch (error) {
    console.error("[nutrition-day] failed", { error: (error as Error).message });
