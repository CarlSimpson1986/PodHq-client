...
import { getOrCreateWorkoutSession } from "@/lib/coach/workout-session";
...
export async function POST(request: Request) {
  ...
  try {
    const { detail, introNarration } = await getOrCreateWorkoutSession(member.id, booking.id, booking.resource_id, member.name);
