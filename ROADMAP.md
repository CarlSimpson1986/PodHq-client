...
8. **Recurring memberships + real PAYG pricing** — added to the roadmap 2026-08-11 at the user's request, after clarifying the actual model: multiple membership tiers, each granting a fixed number of credits per month, billed recurring; unlock access stays exactly as scoped today (a member's own booking, within its time window — `/api/unlock` already enforced this per-booking gate before memberships existed, and memberships don't change it at all, they only affect how credits land in the ledger).
   ...
   **Explicitly not done yet, flagged rather than silently skipped:**
   - Migration 0014 hasn't been applied to Supabase — `/api/checkout-membership` and the webhook's membership handling will error against the live `memberships` table until it is.
   - Production and local (`stripe listen`) webhook endpoints are currently configured for the Stage 4 event set (`checkout.session.completed`, `charge.*`, `payment_intent.*`) — need `customer.subscription.created`/`.updated`/`.deleted` and `invoice.payment_succeeded` added, or Stripe won't deliver them at all.
   - No live test yet: a real test-mode subscription purchase through `/buy-membership`, confirming the `memberships` row lands, first-period credits land, and (ideally) a `stripe events resend` on `invoice.payment_succeeded` doesn't double-credit — same rigor as Stage 4's original credit-pack verification.
   - No cancel/upgrade/downgrade UI — a member can't change or cancel their tier from within this app yet.
