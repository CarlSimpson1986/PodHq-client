# ROADMAP Archive 11 — Weekly Check-in & Training Tab Rework (2026-08-23)
...
`@`-import limit, to make room for Stage 12's training-block
periodization writeup. Covers Stage 10b (the weekly AI-coach check-in)
and the Training tab rework (renamed from Workout, week-by-week
performance graphs, the accordion/consistency layout refinement).
Training-block periodization (Stage 12) continues in the active
`ROADMAP.md`.
...
verification was pending at the time (needed the `0054_check_ins.sql`
migration applied) — since confirmed working live as part of Stage 12c's
verification pass in the active `ROADMAP.md`.
...
app) rather than being a UI change like the two above. (Answered and
built as Stage 12, in the active `ROADMAP.md`.)
