# ROADMAP Archive 10 — Coach Dashboard Stage 10a (2026-08-23)
...
`@`-import limit, to make room for the Training tab layout refinement.
Covers Stage 10a (the Coach dashboard shell, then reworked into a
nested `CoachBottomNav` once Carl clarified what "separate coaching
hub" actually meant, plus the new `/coach/profile` edit page and the
persistent playground test member). Stage 10b (weekly check-in) and the
Training tab rework continue in the active `ROADMAP.md`.
...
history, moved out of the dashboard rather than duplicated (later
renamed to `/coach/training` — see the Training Tab Rework entry).
